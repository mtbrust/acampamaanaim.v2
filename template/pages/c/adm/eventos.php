...
class eventos extends EndPoint
{
	...
	public function loadParams()
	{
		...
		self::$params['scripts']     = [
			// pasta libs.
			'libs' => [
				'calendar/index.global.min.js',   		// Exemplo.
			],
		];
	...
	public function adicionar($params)
	{
		...
		self::$params['html'] = Render::obj('forms/form-eventos.html', $params);
	}
	...
	public function editar($params)
	{
		...
		if (!isset($params['infoUrl']['attr'][1])) {
			...
			// Lista evento específico.
			self::$params['evento'] = Maanaim::listarEvento($id);

			if (empty(self::$params['evento']))
			{
				$this->listarEventos();
			}else {
				self::$params['html'] = Render::obj('forms/form-eventos.html', self::$params);
			}
		}
	}
	...
	public function post($params)
	{
		...
		switch ($funcao) {
			case 'adicionar':
				Maanaim::AdicionarEvento($_POST);

				$this->$funcao($params);
				break;
			case 'editar':
				$id = $params['infoUrl']['attr'][1];
				Maanaim::EditarEvento($id, $_POST);

				$this->$funcao($params);
				// self::$params['html'] = 'Editar';
				break;
	...
	private function listarEventos()
	{
		// Lista eventos.
		self::$params['eventos'] = Maanaim::listarEventos();
		self::$params['html'] = Render::obj('blocos/eventos_cards.html', self::$params);
	}
}
